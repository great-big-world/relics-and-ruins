...
public class RelicsAndRuinsClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(BlockBreakingManager.BlockBreakingInfo.PACKET_ID, (blockBreakingInfo, context) -> {
            BlockPos pos = blockBreakingInfo.pos();
            int entityId = blockBreakingInfo.entityId();
            int damage = blockBreakingInfo.damage();
            int cooldown = blockBreakingInfo.cooldown();
            context.client().execute(() -> {
                if (context.client().world != null) {
                    if (damage == 10) {
                        context.client().interactionManager.breakBlock(pos);
                    }
                    context.client().world.setBlockBreakingInfo(entityId, pos, damage);
                }
            });
        });
    }
